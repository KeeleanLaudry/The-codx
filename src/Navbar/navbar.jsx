...
export default function CodxNavbar() {
  ...
  const serviceDetails = {
    ...
    "Social Media Marketing": {
      ...
      articles: [
        "Why Your Website Is Your Strongest Marketing Tool",
      ]
    },
    "Digital Marketing Services": {
      icon: TrendingUp,
    ...
    "Ads": {
      icon: Megaphone,
      info: ["Paid Ads (Google, Meta, TikTok, YouTube, Snapchat)", "Social Media Campaigns"],
      articles: [
        "The Power of Digital Marketing — Turning Clicks into Conversions",
      ]
    },
    ...
    "ERP": {
      ...
      articles: [
        "How Custom Software Solutions Simplify Business Operations",
      ]
    },
    "IT Resources": {
      icon: Users,
  ...
  const industriesList = [
    { name: "Beauty & Wellness", icon: Sparkles, link: "/beauty-wellness", description: "Digital transformation for modern wellness brands" },
    { name: "Real Estate", icon: Building2, link: "/real-estate", description: "PropTech solutions and virtual experiences" },
    { name: "E-commerce", icon: ShoppingBag, link: "/ecommerce", description: "Scalable platforms and conversion optimization" },
    { name: "Business Consultancy", icon: Briefcase, link: "/business-consultancy", description: "Strategic IT consulting for growth" },
    { name: "Healthcare", icon: HeartPulse, link: "/healthcare", description: "HIPAA-compliant digital health solutions" }
  ];
  ...
  return (
    <div className="fixed top-0 left-0 w-full font-poppins z-50" ref={navbarRef}>
      <div
        className={`flex items-center justify-between  transition-all duration-700 ease-out
        ${
          scrolled
            ? "max-w-[1200px] mx-auto rounded-full py-3 px-6 mt-4 bg-black/60 border border-white/10 shadow-2xl shadow-black/20"
            : "w-full py-5 px-10 bg-gradient-to-b from-black/40 to-transparent"
        }`}
      >
      <Link to="/">
  <img
    src={logo}
    alt="Logo"
    className={`cursor-pointer transition-all duration-500 ${scrolled ? "h-10" : "h-12"} drop-shadow-lg`}
  />
</Link>

        {/* NAV LINKS */}
        <nav
          className={`hidden md:flex items-center gap-10 font-medium text-white/90 transition-all duration-500 ${
            scrolled ? "text-sm" : "text-base"
          }`}
        >
          <Link to="/about" className="relative group py-2">
            <span className="hover:text-white transition-colors">About Codx</span>
          ...
          <div className=" flex items-center" ref={servicesButtonRef}>
            ...
            {servicesOpen && (
              <div
                ...
                // style={{ 
                //   transform: scrolled ? 'translateX(0)' : 'translateX(0)',
                //   left: scrolled ? '-20px' : '-40px'
                // }}
              >
                <div className="grid grid-cols-4 gap-1 p-4 bg-white/5 border-b border-white/10">
                  {Object.keys(services).map((category) => {
                    ...
                    return (
                      ...
                      >
                        {CategoryIcon && <CategoryIcon size={16} className={activeCategory === category ? "text-orange-400" : ""} />}
                ...
                <div className="grid grid-cols-[280px_1fr_340px] gap-8 p-6">
                      ...
                      return (
                        ...
                        >
                          {ServiceIcon && (
                            <ServiceIcon
                              size={16}
                              className={activeService === service ? "text-orange-400" : "text-gray-500 group-hover:text-gray-300"}
                            />
                          )}
                          <span
                            className={`text-sm ${
                              activeService === service ? "text-white font-medium" : "text-gray-400 group-hover:text-gray-200"
                            }`}
                          >
                            {service}
                          </span>
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      {(serviceDetails[activeService]?.info || []).map((item, i) => (
                        <div
                          key={i}
                  ...
                  <div className="border-l border-white/10 pl-6">
                    <div className="flex items-center gap-2 mb-4">
                      ...
                      <h3 className="text-sm font-semibold uppercase tracking-wider text-gray-400">Featured Insight</h3>
                    </div>
                    {(serviceDetails[activeService]?.articles || []).map((article, i) => (
                      <div
                        key={i}
          ...
          <div className=" flex items-center" ref={industriesButtonRef}>
            <div
              ...
              className="flex items-center gap-1.5 cursor-pointer group py-2"
            >
              <span className="group-hover:text-white transition-colors">Industries</span>
              <ChevronDown
                size={14}
            ...
            {industriesOpen && (
              <div
                ref={industriesRef}
                onClick={(e) => e.stopPropagation()}
className="absolute left-1/2 -translate-x-1/2 mt-4 top-full w-[1200px] bg-black/90 backdrop-blur-2xl border border-white/15 rounded-2xl shadow-2xl animate-fadeIn"                style={{ 
                  // transform: scrolled ? 'translateX(0)' : 'translateX(0)',
                  // left: scrolled ? '-20px' : '-40px'
                }}
              >
                <div className="grid grid-cols-3 gap-8 p-8">
                  <div className="col-span-1">
                    ...
                    <p className="text-gray-300 text-sm leading-relaxed">
                      Empowering diverse industries with exclusive IT solutions that drive
        ...
        <div className="flex items-center gap-4">
          <button
            className={`relative overflow-hidden rounded-full border border-white/30 text-white transition-all duration-300 hover:bg-white/10 hover:border-white/50 ${
